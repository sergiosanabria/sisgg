...
class NotaCreditoController extends Controller {
    ...
    public function notasCreditoAction() {
        ...
        $notasCredito = $this->getDoctrine()->getRepository('SisGGFinalBundle:NotaCredito')->findAll();
        $parameters = array("form" => null, 'gestor_autenticacion' => $gestor, 'notasCredito' => $notasCredito);
        return $this->render("SisGGFinalBundle:NotaCredito:notasCredito.html.twig", $parameters);
    }
    ...
    public function anularVentaAction(){
        ...
        if ($this->getRequest()->getMethod() === "POST") {
            ...
            if ($form->isValid()){
                $unaVenta = $this->get("gestor_ventas")->guardarNotaCredito($this->getUser(),$id,$unaVenta);
...
//        $is = array();
//        foreach ($ivas as $iva) {
//            $i = array('iva'=>$iva,'total'=>0.00);
//            foreach ($venta->getLineasVenta() as $linea) {
//                if ($linea->getIvaProductoVenta() == $iva){
//                    $i['total'] = $i['total'] + $linea->getTIVA();
//                }
//            }
//            foreach ($venta->getItemsRecargoVenta() as $linea) {
//                if ($linea->getRecargo()->getIva() == $iva){
//                    $i['total'] = $i['total'] + ($linea->getTotalRecargo() - ($linea->getTotalRecargo()/($linea->getRecargo()->getIva()->getTasa()/100 + 1)));
//                }
//            }
//            $is[ ] = $i;
//        }
//        $format = $this->get('request')->get('_format');
//        $parameters = array("form" => null,'venta'=>$venta,'ivas'=>$is,'empresa' => $this->getEmpresa());
